...
def compute_TracIN_GC_per_iter(model, device, batch_data, validation_loader, 
                               optimizer, trainable_layers, loss_fn, per_val=False):
    ...
    dLdZ_a_val_lst = []
    for step, val_batch in enumerate(validation_loader):
        
        val_input_ids, _ = val_batch
        n_val = val_input_ids.shape[0]

        val_loss = compute_loss(model, device, val_batch, loss_fn)
    ...
    train_loss = compute_loss(model, device, batch_data, loss_fn)
